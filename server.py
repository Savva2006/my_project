...
from store.forms import LoginForm, RegistrationForm, AddGoodsForm, MainForm, \
    AddStoreForm, UpdateGoodsForm
...
def update():
    ...
    if request.method == 'POST':
        ...
        if (db.session.query(db.exists().where(Goods.name ==
                                               request.form['name'],
                                               Goods.store_id == request.form[
                                                   'store_id'],
                                               Goods.id != request.form[
                                                   'id'])).scalar() is False):
            try:
                db.session.commit()
            except Exception:
...
def admin():
    ...
    return render_template('admin.html', goods=goods_data, username=name,
                           store=store, user_id=user_id, form=form)
            ...
            if name != "":
                if price >= 0:
                    if count >= 0:
                        ...
                        flash("Количество для [" + name + "] не должно \
                        быть отрицательным")
                else:
                    flash(
                        "Цена для [" + name + "] не должна быть отрицательной")
            else:
